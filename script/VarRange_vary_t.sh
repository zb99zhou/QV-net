...
display_results() {
    local count=${OPERATIONS_COUNTS["generate pi_i^ar"]}
    local sum_us=${OPERATIONS_SUMS["generate pi_i^ar"]}
    local avg_us=$(echo "scale=6; $sum_us / $count" | bc -l)
    local unit=${OPERATIONS_UNITS["generate pi_i^ar"]}
    ...
    printf "%-25s %-12.6f %-12s\n" "generate pi_i^ar" "$avg_display" "$unit"

    local count=${OPERATIONS_COUNTS["verify pi_i^ar"]}
    local sum_us=${OPERATIONS_SUMS["verify pi_i^ar"]}
    local avg_us=$(echo "scale=6; $sum_us / $count" | bc -l)
    local unit=${OPERATIONS_UNITS["verify pi_i^ar"]}
    ...
    printf "%-25s %-12.6f %-12s\n" "verify pi_i^ar" "$avg_display" "$unit"
    echo "──────────────────────────────────────────────────────"
}
